...
static const char* cublasGetErrorString(cublasStatus_t status) {
    switch (status) {
        ...
        case CUBLAS_STATUS_NOT_INITIALIZED:   return "CUBLAS_STATUS_NOT_INITIALIZED (未初始化，可能是 handle 没建好)";
        case CUBLAS_STATUS_ALLOC_FAILED:      return "CUBLAS_STATUS_ALLOC_FAILED (GPU 内部显存分配失败)";
        case CUBLAS_STATUS_INVALID_VALUE:     return "CUBLAS_STATUS_INVALID_VALUE (传参错误，通常是尺寸/类型对不上)";
        case CUBLAS_STATUS_ARCH_MISMATCH:     return "CUBLAS_STATUS_ARCH_MISMATCH (显卡架构不支持该功能)";
        case CUBLAS_STATUS_MAPPING_ERROR:     return "CUBLAS_STATUS_MAPPING_ERROR (访问 GPU 内存失败)";
        case CUBLAS_STATUS_EXECUTION_FAILED:  return "CUBLAS_STATUS_EXECUTION_FAILED (GPU 程序执行崩溃)";
        case CUBLAS_STATUS_INTERNAL_ERROR:    return "CUBLAS_STATUS_INTERNAL_ERROR (cuBLAS 库内部错误)";
        case CUBLAS_STATUS_NOT_SUPPORTED:     return "CUBLAS_STATUS_NOT_SUPPORTED (不支持的操作，比如奇葩的维度/排布组合)";
...
// use cublaslt
std::vector<float> read_param_fc_w1_cublas(const std::string& path) {
    std::ifstream file(path);
    if (!file) { std::cerr << "Cannot open parameter file: " << path << std::endl; return {}; }
    std::vector<float> params(256 * 128); float param;
    int i = 0;
    while (file >> param) {
        int count = i % 256;
        int nums = i / 256;
        params[count * 128 + nums] = param;
        i ++ ;
    }
    return params;
}
...
// ===================================================================================
    ...
    __global__ void conv1_kernel(/*float *d_conv1_w,float *d_conv1_b,*/float* images_t,
        ...
        for(int i = idx; i < Tile_size_in *Tile_size_in;i+= blocksize){
            ...
            int globel_pix_x = blockIdx.x*blockDim.x + in_row;
            int globel_pix_y = blockIdx.y*blockDim.y + in_col;
    ...
    //block 128 * 1 * 1
    //grid  1 * 1 * 64
    __global__ void fc1_sum2out(float *fc1_sum, float* fc1_out){
        if (threadIdx.x >= 120)
            return;
        int addr = threadIdx.x + blockIdx.z * blockDim.x;
        int addr_out = threadIdx.x + blockIdx.z * 120;
        float sum = fc1_sum[addr];
        float out = sum >= 1.0f ? 1.0f : 0.0f;
        sum = sum * (1.0 - out);
        fc1_out[addr_out] = out;
        fc1_sum[addr] = sum;
    }
...
int main(int argc, char* argv[]) {
    ...
    //if use cublaslt
    auto fc1_w = read_param_fc_w1_cublas(dir + "/fc1.weight.txt");
...
// ===================================================================================
    ...
    // if use cublaslt
    checkCudaErrors(cudaMalloc(&fc1_out, b_size * 120 * sizeof(float)));
    checkCudaErrors(cudaMalloc(&fc1_sum, b_size * 128 * sizeof(float)));
    ...
    //if use cublaslt
    checkCudaErrors(cudaMalloc(&fc1_out_2, b_size * 120 * sizeof(float)));
    checkCudaErrors(cudaMalloc(&fc1_sum_2, b_size * 128 * sizeof(float)));
    ...
    dim3 fc1_cublasLt_block(128,1,1), fc1_cublasLt_grid(1,1,64);

    //-----cuLBASLt------
    cublasLtHandle_t handle1;
    checkCudaErrors(cublasLtCreate(&handle1));

    cublasLtMatmulDesc_t fc3_op_desc;
    checkCudaErrors(cublasLtMatmulDescCreate(&fc3_op_desc, CUBLAS_COMPUTE_32F_FAST_TF32, CUDA_R_32F));
    
    cublasOperation_t transA = CUBLAS_OP_N;
    cublasOperation_t transB = CUBLAS_OP_N;
    cublasLtEpilogue_t epilogue  = CUBLASLT_EPILOGUE_BIAS;
    //cublasLtEpilogue_t bias_struct = CUBLASLT_EPILOGUE_BGRADB;
    cudaDataType_t bias_dt = CUDA_R_32F;
    checkCudaErrors(cublasLtMatmulDescSetAttribute(fc3_op_desc, CUBLASLT_MATMUL_DESC_TRANSA, &transA, sizeof(transA)));
    checkCudaErrors(cublasLtMatmulDescSetAttribute(fc3_op_desc, CUBLASLT_MATMUL_DESC_TRANSB, &transB, sizeof(transB)));
    checkCudaErrors(cublasLtMatmulDescSetAttribute(fc3_op_desc, CUBLASLT_MATMUL_DESC_EPILOGUE, &epilogue, sizeof(epilogue)));
    checkCudaErrors(cublasLtMatmulDescSetAttribute(fc3_op_desc, CUBLASLT_MATMUL_DESC_BIAS_POINTER, &d_fc1_b, sizeof(d_fc1_b)));
    checkCudaErrors(cublasLtMatmulDescSetAttribute(fc3_op_desc, CUBLASLT_MATMUL_DESC_BIAS_DATA_TYPE, &bias_dt, sizeof(bias_dt)));
    
    cublasLtMatrixLayout_t sumdesc ,convdesc ,outdesc;
    checkCudaErrors(cublasLtMatrixLayoutCreate(&sumdesc, CUDA_R_32F, 16 * 4 * 4, 64, 16 * 4 * 4));
    checkCudaErrors(cublasLtMatrixLayoutCreate(&convdesc, CUDA_R_32F, 128, 16 * 4 * 4, 128));
    checkCudaErrors(cublasLtMatrixLayoutCreate(&outdesc, CUDA_R_32F, 128, 64, 128));
    ...
    checkCudaErrors(cudaMalloc(&workspace_2, workspace_size));
    checkCudaErrors(cublasLtMatmulPreferenceSetAttribute(preference, CUBLASLT_MATMUL_PREF_MAX_WORKSPACE_BYTES, &workspace_size, sizeof(workspace_size)));

    int returnresult = 0;
    cublasLtMatmulHeuristicResult_t heuristic_resuit = {};
    checkCudaErrors(cublasLtMatmulAlgoGetHeuristic(handle1, fc3_op_desc, convdesc, sumdesc, outdesc, outdesc, preference, 1, &heuristic_resuit, &returnresult));
    if(returnresult == 0){
        std::cout << "no return result" << std::endl;
    }
    ...
    cudaGraphExec_t init_gra1, init_gra2; 
    bool is_graph1_work = false, is_graph2_work = false; 
    float alpha = 1.0f, beta_one = 1.0f, beta_zero = 0.0f;
        ...
        {     
            ...
            if(is_graph1_work == false){
                checkCudaErrors(cudaStreamBeginCapture(stream1,cudaStreamCaptureModeGlobal));
                    ...
                    conv1_kernel<<<conv1_grid,conv1_block,20 * 20 * 4,stream1>>>(d_mem_A, conv1_sum_out, conv1_ReLU_out, Loop_times);
                    Maxpool1<<<Maxpool1_grid,Maxpool1_block,0,stream1>>>(conv1_ReLU_out, Maxpool1_out);

                    conv2_kernel<<<conv2_grid,conv2_block,6 * 12 * 12 * 4,stream1>>>(Maxpool1_out, conv2_sum_out, conv2_ReLU_out, Loop_times);
                    Maxpool2<<<Maxpool2_grid,Maxpool2_block,0,stream1>>>(conv2_ReLU_out, Maxpool2_out);

                    //fc1<<<fc1_grid,fc1_block,0,stream1>>>(Maxpool2_out, d_fc1_w, d_fc1_b, fc1_sum, fc1_out, Loop_times);
                    float *beta = (Loop_times == 0)? &beta_zero : &beta_one;
                    cublasLtMatmul(
                        handle1,
                        fc3_op_desc,
                        &alpha,
                        d_fc1_w, convdesc,
                        Maxpool2_out, sumdesc,
                        beta,
                        fc1_sum, outdesc,
                        fc1_sum, outdesc,
                        &heuristic_resuit.algo,
                        workspace,
                        workspace_size,
                        stream1
                        );
                    fc1_sum2out<<<fc1_cublasLt_grid,fc1_cublasLt_block,0,stream1>>>(fc1_sum, fc1_out);
                    fc2<<<fc2_grid,fc2_block,0,stream1>>>(fc1_out, d_fc2_w, d_fc2_b, fc2_sum, fc2_out, Loop_times);
                    fc3<<<fc3_grid,fc3_block,0,stream1>>>(fc2_out, d_fc3_w, d_fc3_b, fc3_sum, fc3_out, Loop_times);
                    }
                checkCudaErrors(cudaStreamEndCapture(stream1, &graph1));
                checkCudaErrors(cudaGraphInstantiate(&init_gra1, graph1, 0));
                is_graph1_work = true;
            }
        checkCudaErrors(cudaGraphLaunch(init_gra1, stream1));
        checkCudaErrors(cudaMemcpyAsync(labels_set(final_data, b_size, images_group), fc3_out, b_size * 10 * sizeof(float), cudaMemcpyDeviceToHost,stream1));
        printf("batch %d -> stream1\n", images_group);

        }else{
            checkCudaErrors(cudaMemcpyAsync(d_mem_B,now_p,(b_size * col * row) * sizeof(float),cudaMemcpyHostToDevice,stream2));
            if(is_graph2_work == false){
                checkCudaErrors(cudaStreamBeginCapture(stream2,cudaStreamCaptureModeGlobal));
                for(int Loop_times = 0; Loop_times < total_loop_times; Loop_times ++){
                    conv1_kernel<<<conv1_grid,conv1_block,20 * 20 * 4,stream2>>>(d_mem_B, conv1_sum_out_2, conv1_ReLU_out_2, Loop_times);
                    Maxpool1<<<Maxpool1_grid,Maxpool1_block,0,stream2>>>(conv1_ReLU_out_2, Maxpool1_out_2);

                    conv2_kernel<<<conv2_grid,conv2_block,6 * 12 * 12 * 4,stream2>>>(Maxpool1_out_2, conv2_sum_out_2, conv2_ReLU_out_2, Loop_times);
                    Maxpool2<<<Maxpool2_grid,Maxpool2_block,0,stream2>>>(conv2_ReLU_out_2, Maxpool2_out_2);

                    //fc1<<<fc1_grid,fc1_block,0,stream2>>>(Maxpool2_out_2, d_fc1_w, d_fc1_b, fc1_sum_2, fc1_out_2, Loop_times);
                    float *beta = (Loop_times == 0)? &beta_zero : &beta_one;
                    cublasLtMatmul(
                        handle1,
                        fc3_op_desc,
                    ...
                    fc1_sum2out<<<fc1_cublasLt_grid,fc1_cublasLt_block,0,stream2>>>(fc1_sum_2, fc1_out_2);
                    fc2<<<fc2_grid,fc2_block,0,stream2>>>(fc1_out_2, d_fc2_w, d_fc2_b, fc2_sum_2, fc2_out_2, Loop_times);
                    fc3<<<fc3_grid,fc3_block,0,stream2>>>(fc2_out_2, d_fc3_w, d_fc3_b, fc3_sum_2, fc3_out_2, Loop_times);

                }
                checkCudaErrors(cudaStreamEndCapture(stream2, &graph2));
                checkCudaErrors(cudaGraphInstantiate(&init_gra2, graph2, 0));
                is_graph2_work = true;
            }
            checkCudaErrors(cudaGraphLaunch(init_gra2, stream2));
            checkCudaErrors(cudaMemcpyAsync(labels_set(final_data, b_size, images_group), fc3_out_2, b_size * 10 * sizeof(float), cudaMemcpyDeviceToHost,stream2));
            printf("batch %d -> stream2\n", images_group);
        }
}
...
//}
    ...
    if (is_graph1_work) { checkCudaErrors(cudaGraphExecDestroy(init_gra1)); checkCudaErrors(cudaGraphDestroy(graph1)); }
    if (is_graph2_work) { checkCudaErrors(cudaGraphExecDestroy(init_gra2)); checkCudaErrors(cudaGraphDestroy(graph2)); }
    ...
    float seconds;
    cudaEventElapsedTime(&seconds, start1, stop);
